...
export class AuthService {
    ...
    async register(username: string, email: string, password: string, confirmPassword: string) {
        ...
        if (password == confirmPassword) {
            const encryptEmail = this.encryptionService.encrypt(email);
            // this.logger.log(encryptEmail);
            const result = await this.usersService.create({ username, email: encryptEmail, password: hashedPassword, confirmPassword: HashedPassword, role_id: '566bb6b2-a7b1-47ad-a4f6-ea730944a44b', otp: hashedOTP, otp_expires_at: otp_expires_at });
            const message = `This is your otp ${otp}`;
            this.mailService.sendMail({
                from: 'Hamza abbas <abbashamza59099@gmail.com>',
                to: email,
